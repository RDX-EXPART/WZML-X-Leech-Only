...
async def start(_, message):
    ...
    if len(message.command) > 1 and message.command[1] == "wzmlx":
        ...
        if decrypted_url.startswith("file"):
            return await send_message(
                message,
                "<b>Bot PM file delivery is disabled.</b>",
            )
    ...
    if await CustomFilters.authorized(_, message):
        ...
        await send_message(
            message,
            "<i>This is a leech-only bot. Files are uploaded only to each user's saved dump channel.\n\n⚠️ You are not an authorized user.</i>",
            reply_markup,
            photo="IMAGES",
        )
