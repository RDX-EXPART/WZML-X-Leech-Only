...
class Mirror(TaskListener):
    ...
    async def new_event(self):
        if not self.user_dict.get("LEECH_DUMP_CHAT"):
            buttons = ButtonMaker()
            buttons.data_button("Set Dump", f"userset {self.user_id} setdump")
            await send_message(
                self.message,
                """<b>Dump Channel Required!</b>

You must set a dump channel before starting a leech task. Add this bot as an administrator in your channel, then tap <b>Set Dump</b> and save the channel ID or @username.

<i>You cannot leech any file until a dump channel is set.</i>""",
        ...
        if args.get("-up"):
            await send_message(
                self.message,
                "Custom upload destinations are disabled. Files are always sent to your saved dump channel.",
            )
            return
