...
class Config:
    ...
    def set(cls, key, value):
        if hasattr(cls, key):
            if key in cls._LEECH_ONLY_VALUES:
                value = cls._LEECH_ONLY_VALUES[key]
    ...
    def load(cls):
        cls.load_config()
        cls.load_env()
        cls._enforce_leech_only()

    @classmethod
    def _enforce_leech_only(cls):
        for key, value in cls._LEECH_ONLY_VALUES.items():
            setattr(cls, key, value)
