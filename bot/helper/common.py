...
class TaskConfig:
    def __init__(self):
        ...
        # Leech-only policy: files must never be copied to a user's Bot PM.
        self.bot_pm = False
        self.file_details = {}
        self.mode = tuple()
    ...
    def _set_mode_engine(self):
        ...
        in_mode = f"#{'Mega' if self.is_mega else 'qBit' if self.is_qbit else 'SABnzbd' if self.is_nzb else 'JDown' if self.is_jd else 'RCloneDL' if self.is_rclone else 'GDrive' if self.is_gdrive else 'Aria2' if (self.source_url and self.source_url != self.message.link) else 'TgMedia'}"
    ...
    async def before_start(self):
        if not self.is_leech:
            raise ValueError("Only leech tasks are supported in this build.")
        ...
        # A user's saved dump is the only allowed upload destination.
        self.bot_pm = False

        if self.bot_trans:
            self.transmission_mode = "bot"
        elif self.user_trans:
            self.transmission_mode = "user"
        elif self.hybrid_leech:
            self.transmission_mode = "both"

        self.up_dest = self.user_dict.get("LEECH_DUMP_CHAT")
        if not self.up_dest:
            raise ValueError(
                "Dump Channel Required! Open /usetting, tap Set Dump, and set "
                "your dump channel before starting a leech task."
            )
        ...
        try:
            ...
            raise ValueError(
                "Dump channel not found. Add the bot as an administrator and set it again."
            ) from error
        if chat.type not in allowed_chats:
            raise ValueError("Dump destination must be a channel or group; Bot PM is disabled.")
        ...
        if chat.type == ChatType.CHANNEL and not getattr(
            ...
            raise ValueError("Give the bot permission to post messages in the dump channel.")
        ...
        if self.transmission_mode in ("user", "both"):
            if not TgClient.user:
                ...
                try:
                    user_chat = await TgClient.user.get_chat(self.up_dest)
                    user_member = await user_chat.get_member(TgClient.user.me.id)
                    user_privileges = getattr(user_member, "privileges", None)
                    if user_chat.type not in allowed_chats or not user_privileges:
                        self.transmission_mode = "bot"
                    elif not getattr(user_privileges, "can_delete_messages", False):
                        self.transmission_mode = "bot"
                except Exception:
                    self.transmission_mode = "bot"
