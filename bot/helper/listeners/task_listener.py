 ...
 """,
    ...
    async def on_download_complete(self):
        ...
        if not self.is_leech:
            await self.on_upload_error("Only Telegram leech uploads are supported.")
            return
    ...
    async def on_upload_complete(
        ...
        if not files and not self.is_super_chat:
            ...
            file_message = ""
            for index, (file_link, name) in enumerate(files.items(), start=1):
                file_message += f"{index}. <a href='{file_link}'>{name}</a>"
                file_message += "\n"
                if len(file_message.encode() + msg.encode()) > 4000:
                    await send_message(self.message, msg + file_message)
                    await sleep(1)
                    file_message = ""
            if file_message:
                await send_message(self.message, msg + file_message)
