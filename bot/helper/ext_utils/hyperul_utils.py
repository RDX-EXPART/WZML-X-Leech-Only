...
class HypertgUpload(HypertgTransfer):
    ...
    async def upload(
        ...
        if not is_image and thumb is None:
            ...
            thumb_path = f"{base_path}/auto-thumb/{file_name}.jpg"
            if await aiopath.isfile(thumb_path):
                thumb = thumb_path
            elif await aiopath.isfile(thumb_path.replace("/auto-thumb", "")):
                thumb = thumb_path.replace("/auto-thumb", "")
        ...
        if not upload_chat_id:
            raise ValueError("Dump channel is not set.")
        try:
            if use_hyper:
                hyper_rply = (
                    reply_to_message_id
