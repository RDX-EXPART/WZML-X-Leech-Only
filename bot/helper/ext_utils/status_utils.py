...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    for index, task in enumerate(
        ...
        if not msg:
            msg = "🔸 <b>RDX LEECH STATUS</b> 🔹\n\n"
        elif index > 1:
            msg += "\n"

        task_no = index + start_position
        msg += f"╭ <b>File {task_no}</b> » <i>{escape(f'{task.name()}')}</i>"
        if task.listener.subname:
            msg += f"\n├ <b>Sub Name</b> » <i>{escape(task.listener.subname)}</i>"
        ...
        if (
            ...
            msg += f"\n├ <b>Progress</b> » {get_progress_bar_string(progress)} <i>{progress}</i>"
            if task.listener.subname:
        ...
        msg += f"\n├ <b>User</b> » {user.mention(style='html')}"
    ...
    if len(msg) == 0:
        if status == "All":
            ...
            msg = f"🔸 <b>RDX LEECH STATUS</b> 🔹\n\n╰ No active {status} tasks.\n"
    ...
    if len(tasks) > STATUS_LIMIT:
        ...
        buttons.data_button(
            f"{page_no}/{pages}", f"status {sid} ref", position="header"
        )
    ...
    buttons.data_button(
        "♻️ Refresh", f"status {sid} ref", style=ButtonStyle.PRIMARY
    )
    button = buttons.build_menu(2, h_cols=3, f_cols=4)
    disk = disk_usage(DOWNLOAD_DIR)
    msg += f"\n╭ <b>CPU</b> » {cpu_percent()}% | <b>FREE</b> » {get_readable_file_size(disk.free)}"
    msg += f"\n├ <b>RAM</b> » {virtual_memory().percent}% | <b>UP</b> » {get_readable_time(time() - bot_start_time)}"
    msg += f"\n╰ <b>Tasks</b> » {tasks_no} | <b>Page</b> » {page_no}/{pages}"
    return msg, button
